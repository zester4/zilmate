      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <section className="bg-white rounded-2xl p-6 shadow-xl shadow-brand-rose/5 border border-brand-rose/5 flex flex-col gap-4">
          ...
          <textarea
            ...
            className="flex-1 min-h-[150px] bg-brand-peach/30 border-none rounded-xl p-4 text-sm focus:ring-2 focus:ring-brand-rose/20 outline-none resize-none"
        ...
        <section className="bg-white rounded-2xl p-6 shadow-xl shadow-brand-rose/5 border border-brand-rose/5 flex flex-col gap-4">
          ...
          <div className="flex-1 flex flex-col gap-3 overflow-y-auto min-h-[200px]">
            {memories.map((m, i) => (
              <div key={i} className="p-4 bg-brand-peach/20 rounded-xl border border-brand-rose/5 text-sm leading-relaxed">
                {m.text}
              </div>
            ))}
            {memories.length === 0 && !isPending && <div className="text-gray-300 text-center mt-10 italic">No memories found.</div>}
          </div>
        </section>
      </div>
    </main>
