      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {departments.map((dept) => (
          <section key={dept.name} className="bg-white rounded-2xl p-6 shadow-xl shadow-brand-rose/5 border border-brand-rose/5 flex flex-col gap-4">
            ...
            <p className="text-sm text-[#4A4543] line-clamp-3 leading-relaxed">{dept.mission}</p>
            <div className="mt-4 flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                {dept.specialists.map(s => (
                  <span key={s} className="bg-brand-peach/50 text-[#1E1B19] text-[10px] font-medium px-3 py-1.5 rounded-lg border border-brand-rose/5">{s}</span>
                ))}
              </div>
            </div>
